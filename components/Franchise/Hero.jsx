...
const Hero = () => {
    ...
    return (
        <section className='bg-orangeColorBackground'>
            <div className="max-w-screen-xl mx-auto px-4 py-10 gap-12 text-gray-600 md:px-8 xl:flex xl:py-20">
                <div className="space-y-5 max-w-2xl mx-auto text-center xl:text-left">
                    <div className="flex flex-wrap items-center justify-center gap-6 xl:justify-start">
                        {
                            features.map((item, idx) => (
                                <div key={idx} className="flex items-center gap-x-2 text-gray-800 text-sm">
                                    {item.icon}
                                    {item.name}
                                </div>
                            ))
                    ...
                    <h1 className="text-4xl text-gray-800 font-extrabold mx-auto md:text-5xl 2xl:leading-tight ">
                    <span className='text-black/80'>Join</span> 
                    {" "}
                    <span className='text-orange-700'>MUGHALS</span>
                    {" "}
                    <span className='text-black/80'>
                    Your Recipe for Franchise Success
                    </span>
                    </h1>
                    <p className="max-w-xl mx-auto xl:mx-0 font-semibold leading-relaxed">
                        Sed ut perspiciatis unde omnis iste natus voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae.
                    </p>
                    <div className="items-center justify-center gap-x-3 space-y-3 sm:flex sm:space-y-0 xl:justify-start">
                        <Link href="contact" className="flex items-center justify-center gap-x-2 py-2 px-4 text-white bg-gradient-to-bl from-orange-600 bg-orange-400 font-medium rounded-lg md:inline-flex">
                            Get a Franchise
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
